const NftCard = ({ image , id, title, address, description, attributes }) => {
  return (
      <div className="w-1/4 mr-3 mb-4 bg-slate-100 rounded-md" >
          <img className='w-full rounded-t-md' key={id} src={image} alt="NFTs"></img>
          <div className="p-3">
              <div className="flex mb-3">
                  ...
                  <div className="flex mr-3">
                      <a target="_blank" rel="noreferrer" className="text-blue-700" href={`https://rinkeby.etherscan.io/token/${address}`}>{`${address.slice(0, 4)}...${address.slice(address.length - 4)}`}</a>
                  </div>
              </div>
